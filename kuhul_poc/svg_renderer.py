...
def render_svg(state: Dict[str, Any], *, width: int = 900, height: int = 520) -> str:
    ...
    parts.append(
        ...
        f'viewBox="0 0 {width} {height}">'  # noqa: W503
    )
    ...
    parts.append(
        ...
        f'font-family="ui-sans-serif,system-ui" font-size="18" fill="{accent}">'
        f'{_esc("KUHUL PoC — CLI ⇄ SVG")}</text>'
    )
    parts.append(
        f'<text x="{x0 + pad}" y="{y0 + pad + 40}" '
        f'font-family="ui-sans-serif,system-ui" font-size="12" fill="{text}">'
        f'{_esc("Theme: " + str(theme))}</text>'
    )
    ...
    if not comps:
        parts.append(
            ...
            f'font-family="ui-sans-serif,system-ui" font-size="13" fill="{text}">'
            f'{_esc("No components yet. Use CLI: create button --text=OK")}</text>'
        )
    else:
        for i, comp in enumerate(comps):
            ...
            parts.append(
                f'<rect x="{list_x}" y="{cy - 18}" width="{w}" height="44" '
                f'rx="12" fill="none" stroke="{stroke}" opacity="0.55"/>'
            )
            parts.append(
                f'<text x="{list_x + 14}" y="{cy + 8}" '
                f'font-family="ui-sans-serif,system-ui" font-size="13" fill="{text}">'
                f'{_esc(f"{ctype}  ({cid})")}</text>'
            )
            if label:
                parts.append(
                    f'<text x="{list_x + w - 14}" y="{cy + 8}" text-anchor="end" '
